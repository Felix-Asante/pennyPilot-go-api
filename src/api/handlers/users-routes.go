...
func (h *UsersHandler) SetupRoutes() {
	...
	router.Route("/users", func(route chi.Router) {
		...
		route.Get("/{userId}/accounts/total-savings", usersRoutesHandler.totalSavings)
		...
		route.Get("/{userId}/expenses/total", usersRoutesHandler.getTotalExpenses)
		route.Get("/{userId}/expenses/categories", usersRoutesHandler.getExpenseCategories)
		route.Get("/{userId}/financial-obligations", usersRoutesHandler.getFinancialObligations)
